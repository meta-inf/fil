...
def add_intercept(a):
    ...
    return np.concatenate([a, np.ones((a.shape[0], 1))], -1)


def batched_apply(apply_fn, inp: np.ndarray, B=2000) -> np.ndarray:
    return np.concatenate(
        [apply_fn(inp[i: i+B]) for i in range(0, inp.shape[0], B)],
        0)


def reset_xla_memory():
...
def dict_subset(dct, keys, optional_keys=[]):
    ret = dict((k, dct[k]) for k in keys)
    ret.update((k, dct[k]) for k in optional_keys if k in dct)
    return ret


def to_py_dict(dct) -> dict:
    def sanitize(a):
        try:
            ret = float(a)
        except:
            if isinstance(a, (np.ndarray, onp.ndarray)):
                ...
                ret = a
        return ret
    return jax.tree_util.tree_map(sanitize, dct)
